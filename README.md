...
- 현재 단계: `v0.1.0` 구현 완료 및 PR 준비
- 현재 목표 버전: `v0.1.0` 마무리
- 다음 작업: 현재 브랜치 push 후 PR 생성
- 다음 목표 버전: `v0.2.0` 드래그 이동과 크기 조절
- MVP 방향: 버튼 추가, 선택, HTML 코드 실시간 생성부터 시작

## 다음 버전 준비

`v0.2.0` 구현은 `v0.1.0` PR이 `develop`에 반영된 뒤 새 브랜치에서 시작한다.

진행 순서:

1. 현재 `feature/v0.1.0-react-init` 브랜치를 push한다.
2. `develop`을 base로 `v0.1.0` PR을 만든다.
3. PR 검토 후 merge한다.
4. 로컬 `develop`을 최신 상태로 pull 받는다.
5. `v0.2.0` 작업 브랜치를 새로 만든다.
6. 새 브랜치에서 드래그 이동과 크기 조절 구현을 시작한다.
