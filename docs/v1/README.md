...
- [v1.3 기획안](v1_3_plan.md): 컨테이너 중심 구조, 애니메이션 미리보기, 도킹 작업 패널, 꾸미기 확장 계획
...
2. 세부 버전 기획이 있으면 `v1_3_plan.md`처럼 버전별 기획안을 확인한다.
3. `data_rules.md`에서 필요한 데이터 형태를 먼저 확인한다.
4. `file_structure.md`에서 파일 위치를 정한다.
5. `checklist.md`에 구현할 항목을 체크 가능한 단위로 남긴다.
6. 구현이 끝난 뒤 필요한 문서만 갱신하고 PR을 만든다.
