...
- `locked`가 true인 컴포넌트는 캔버스에서 선택, 이동, 리사이즈, 속성 수정이 되지 않는다.
- 고정된 컴포넌트는 우클릭 메뉴나 작업 패널 잠금 탭에서 고정 해제할 수 있다.
- 고정 상태는 project JSON에 저장한다.

## 기준 스타일 규칙

project JSON은 새 컴포넌트 생성 기준이 되는 `styleDefaults`를 가질 수 있다.
...
- 새 컴포넌트를 만들 때 해당 타입의 기준 스타일을 자동 적용한다.
- 작업 패널 기준 탭에서 선택 컴포넌트의 현재 스타일을 기준으로 등록할 수 있다.
- 선택 컴포넌트에 기준 스타일을 다시 적용할 수 있다.
- 기준 초기화는 기본 타입별 스타일로 되돌린다.
