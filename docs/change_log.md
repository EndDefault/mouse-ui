...
- v1.2까지는 캔버스 조작, 다중 선택, 우클릭 삭제, 선택 HTML 강조 같은 사용성 개선을 중심으로 진행했다.

변경 기준:

- v1.3은 컨테이너 중심 구조, 애니메이션 미리보기, 도킹 작업 패널, 꾸미기 기능 확장을 다음 목표로 둔다.
- 기존 `box`는 컨테이너와 역할이 겹치므로 제거하고, 불러오기 시 `container`로 변환한다.
- project JSON과 workspace settings를 분리해 UI 데이터와 작업 환경 데이터를 따로 관리한다.

변경 이유:

- UI 빌더를 단순 배치 도구에서 실제 화면 설계 도구로 확장하기 위해
- 오른쪽 고정 사이드바를 더 유연한 작업 패널로 바꾸기 위해
- 그라데이션, 그림자, border, 모서리 같은 스타일 편집을 세밀하게 만들기 위해
