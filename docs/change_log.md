...
- 잠금 컴포넌트 위에서도 캔버스 배경처럼 드래그 선택 범위가 표시되도록 라쏘 선택 조건을 보정했다.
- 작업 패널에 `그룹` 탭을 추가했다.
- 선택된 컴포넌트를 같은 부모 프레임 단위로 보여주도록 그룹 패널을 구성했다.
- 그룹 패널에서 가로 정렬, 세로 정렬, 가로 간격, 세로 간격 맞추기를 실행할 수 있게 했다.
- 상단 버전 표시와 패키지 버전을 `v1.4.2`로 변경했다.
